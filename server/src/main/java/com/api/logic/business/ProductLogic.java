...
public class ProductLogic {
  ...
  public ArrayList<GetRankingResponse> mostUsedByProposal(User loggedUser, int amount) throws ApiException {
    try {
      ArrayList<Ranking> products = null;

      if((loggedUser.getOrganization().getRole().equals(OrganizationRoles.SUPPLIER))) {
        products = pda.mostUsedByProposal(loggedUser.getOrganization().getId(), amount);
      }
      else {
        products = pda.topSellersByOrder(loggedUser.getOrganization().getId(), amount);
      }
